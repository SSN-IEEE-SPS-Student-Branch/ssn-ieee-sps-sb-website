...
import { motion, useMotionValue, useSpring, useTransform, AnimatePresence, useInView, useReducedMotion, Variants } from 'framer-motion';
    ...
    <div style={{
      ...
      <AnimatePresence initial={false} custom={direction}>
        <MotionImage
          ...
          alt={`SSN IEEE SPS chapter activity ${imageIndex + 1} of ${images.length}`}
          fill
      ...
      <button type="button" aria-label="Show previous chapter activity" onClick={() => paginate(-1)} style={{ position: 'absolute', left: '10px', zIndex: 2, background: 'rgba(0,0,0,0.4)', color: 'white', border: 'none', borderRadius: '50%', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backdropFilter: 'blur(4px)' }}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
      </button>
      <button type="button" aria-label="Show next chapter activity" onClick={() => paginate(1)} style={{ position: 'absolute', right: '10px', zIndex: 2, background: 'rgba(0,0,0,0.4)', color: 'white', border: 'none', borderRadius: '50%', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backdropFilter: 'blur(4px)' }}>
...
export default function AboutPage() {
  ...
  return (
    <div style={{ width: '100%', overflowX: 'hidden', background: 'transparent', color: '#ffffff', minHeight: '100vh' }}>
